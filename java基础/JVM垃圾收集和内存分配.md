...
## 内存分配策略 ##
- **对象优先分配在Eden**。Eden没有足够空间，虚拟机进行一次Minor GC。
- **大对象直接进入老年代**。典型的是很长的字符串或者大数组。
- **长期存活的对象将进入老年代**。每个对象都有一个**年龄计数器**，对象在Eden出生并经过一次Minor GC后存活，并且能被Survivor容纳，将进入Survivor。在Survivor每经过一次Minor GC，年龄加1，年龄增加到阈值（默认15），则进入老年代。
...
- 逃逸状态：
  ...
  - 参数逃逸。对象作为方法参数传递。
  - 没有逃逸。
- [逃逸分析优点](https://segmentfault.com/a/1190000038262877)：
  - 锁消除。
  - 分配在栈上。
- 逃逸分析缺点：
